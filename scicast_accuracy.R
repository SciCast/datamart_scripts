...
# acop is the Brier score on each question.  Running this file should put the means (over questions) of three forms of accuracy for edits on the screen.
acqum <- mean(acqu); acunm <- mean(acun); print(acqum); print("Brier Score")
pocosm<-mean(pocos,na.rm=T); pocoum<-mean(pocou,na.rm=T); print(pocosm); print("Percentage on correct option")
hitm <- mean(hit,na.rm=T); print(hitm); print("Hit rate") # Also compare to pocoum.
